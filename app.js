...
app.get('/', async(req, res) => {
    ...
    let mostSellingProduct = null;
    let mostSellingProductQuantity = 0;
    if (Object.keys(productSales).length > 0) {
        mostSellingProduct = Object.keys(productSales).reduce((a, b) => productSales[a] > productSales[b] ? a : b);
        mostSellingProductQuantity = productSales[mostSellingProduct];
    }

    const sortedProductSales = Object.entries(productSales).sort((a, b) => b[1] - a[1]);
    const top5Products = sortedProductSales.slice(0, 5);
    res.render('layout/dashboard/report.ejs', {
        totalCustomers,
        totalOrders,
        totalAmount,
        mostSellingProduct,
        mostSellingProductQuantity,
        monthlySales,
